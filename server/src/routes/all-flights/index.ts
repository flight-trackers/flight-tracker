import { IFlightData, IOpenSkyRawData } from "src/interfaces/IOpenSkyData";
import { IState } from "src/interfaces/IOpenSkyData";
...
const allFlights: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  ...
  fastify.get("/", async function (request, reply) {
    ...
    const mapData: IState[] = resp.data.states.map(
      (flight: IOpenSkyRawData[]) => {
        return {
          icao24: flight[0],
          callsign: flight[1],
